...
public class BmcDataStoreFactory {
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(BmcDataStoreFactory.class);
     ...
     */
    public BmcDataStore createDataStore(final String namespace, final String bucket, final Statistics statistics) {
        ...
        final BmcPropertyAccessor propertyAccessor = new BmcPropertyAccessor(this.configuration, propertyOverrideSuffix);
    ...
    ObjectStorage createClient(final BmcPropertyAccessor propertyAccessor) {
        final String customObjectStoreClient = propertyAccessor.asString().get(BmcProperties.OBJECT_STORE_CLIENT_CLASS);
        final ObjectStorage objectStorage = (customObjectStoreClient != null) ? (ObjectStorage) this.createClass(customObjectStoreClient) : buildClient(propertyAccessor);
    ...
    private ObjectStorage buildClient(final BmcPropertyAccessor propertyAccessor) {
        final ClientConfiguration.ClientConfigurationBuilder clientConfigurationBuilder = ClientConfiguration.builder();
        final Integer connectionTimeoutMillis = propertyAccessor.asInteger().get(BmcProperties.CONNECTION_TIMEOUT_MILLIS);
        ...
        final Integer readTimeoutMillis = propertyAccessor.asInteger().get(BmcProperties.READ_TIMEOUT_MILLIS);
        ...
        clientConfigurationBuilder.retryConfiguration(RetryConfiguration.builder().terminationStrategy(MaxTimeTerminationStrategy.ofSeconds(retryTimeoutInSeconds)).delayStrategy(new ExponentialBackoffDelayStrategy(Duration.ofSeconds(retryTimeoutInSeconds).toMillis())).build());
        final ClientConfiguration clientConfig = clientConfigurationBuilder.build();
        final BasicAuthenticationDetailsProvider authDetailsProvider = this.createAuthenticator(propertyAccessor);
        ...
        return (StringUtils.isBlank(httpProxyUri)) ? new ObjectStorageClient(authDetailsProvider, clientConfig) : buildClientWithProxy(authDetailsProvider, clientConfig, propertyAccessor, httpProxyUri);
    }

    private ObjectStorage buildClientWithProxy(final BasicAuthenticationDetailsProvider authDetailsProvider, final ClientConfiguration clientConfig, final BmcPropertyAccessor propertyAccessor, final String httpProxyUri) {
        LOG.info("Setting HTTP Proxy URI to {}", httpProxyUri);
        final ClientConfigDecorator poolConfigDecorator = newApacheConnectionPoolingClientConfigDecorator(clientConfig);
        final ClientConfigDecorator proxyConfigDecorator = newApacheProxyConfigDecorator(httpProxyUri, propertyAccessor);
        final List<ClientConfigDecorator> clientConfigDecorators = Lists.newArrayList(poolConfigDecorator, proxyConfigDecorator);
        final ClientConfigurator clientConfigurator = new ApacheConfigurator(clientConfigDecorators);
        return ObjectStorageClient.builder().configuration(clientConfig).clientConfigurator(clientConfigurator).build(authDetailsProvider);
    }
    ...
    private ClientConfigDecorator newApacheConnectionPoolingClientConfigDecorator(final ClientConfiguration clientConfig) {
        ...
        ApacheConnectionPoolConfig.builder().defaultMaxConnectionsPerRoute(maxConns).totalOpenConnections(maxConns).ttlInMillis(clientConfig.getConnectionTimeoutMillis()).build();
        return new ApacheConnectionPoolingClientConfigDecorator(poolConfig);
    }

    private ClientConfigDecorator newApacheProxyConfigDecorator(final String httpProxyUri, final BmcPropertyAccessor propertyAccessor) {
        final ApacheProxyConfig proxyConfig = ApacheProxyConfig.builder().uri(httpProxyUri).username(propertyAccessor.asString().get(BmcProperties.HTTP_PROXY_USERNAME)).password(propertyAccessor.asString().get(BmcProperties.HTTP_PROXY_PASSWORD)).build();
    ...
    private void setConnectorVersion() {
        ...
        try {
            propertyStream = getClass().getClassLoader().getResourceAsStream("com/oracle/bmc/hdfs/" + OCI_PROPERTIES_FILE_NAME);
            ...
            ClientRuntime.setClientUserAgent(String.format("Oracle-HDFS_Connector/%s", connectorVersion));
        } finally {
    ...
    BasicAuthenticationDetailsProvider createAuthenticator(BmcPropertyAccessor propertyAccessor) {
        final String customAuthenticator = propertyAccessor.asString().get(BmcProperties.OBJECT_STORE_AUTHENTICATOR_CLASS);
        ...
        return SimpleAuthenticationDetailsProvider.builder().tenantId(tenantId).userId(userId).fingerprint(fingerprint).privateKeySupplier(supplier).passphraseCharacters(passPhrase).build();
    }
    ...
    private <T> T createClass(final String className) {
        try {
            ...
            final Constructor<?> customClassConstructor = customClass.getConstructor(Configuration.class);
            try {
    ...
    public BmcDataStoreFactory(final Configuration configuration) {
