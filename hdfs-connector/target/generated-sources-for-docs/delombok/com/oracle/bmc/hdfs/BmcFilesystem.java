...
public class BmcFilesystem extends FileSystem {
    private static final org.slf4j.Logger LOG = org.slf4j.LoggerFactory.getLogger(BmcFilesystem.class);
    ...
    public void initialize(URI uri, final Configuration configuration) throws IOException {
        ...
        this.dataStore = new BmcDataStoreFactory(configuration).createDataStore(namespace, bucket, super.statistics);
        ...
        LOG.info("Setting working directory to {}, and initialized uri to {}", this.workingDirectory, this.uri);
    }
     ...
     */
    @Override
    public FSDataOutputStream create(final Path path, final FsPermission permission, final boolean overwrite, final int bufferSize, final short replication, final long blockSize, final Progressable progress) throws IOException {
        LOG.debug("Attempting to create path {}, overwrite {}, bufferSize {}", path, overwrite, bufferSize);
        ...
        if (existingFile != null) {
            ...
            LOG.debug("No existing file at path {}, verifying all directories exist with mkdirs", path);
            // no existing file, so make sure all of the parent "directories"
            // are created
            this.mkdirs(path.getParent(), permission);
        }
        return new FSDataOutputStream(this.dataStore.openWriteStream(path, bufferSize, progress), super.statistics);
    }
    ...
    public FSDataInputStream open(final Path path, final int bufferSize) throws IOException {
        ...
        return new FSDataInputStream(this.dataStore.openReadStream(status, path, bufferSize, super.statistics));
    }
    ...
    public boolean rename(final Path source, final Path destination) throws IOException {
        ...
        if (destinationStatus == null) {
            final FileStatus destinationParentStatus = this.getNullableFileStatus(absoluteDestination.getParent());
